...
# Configuração da página do Portal - Forçando o uso de toda a largura da tela
...
def calcular_vencimento_e_alertas(linha):
    frete_a_pagar = linea_get = linha.get('Falta Pagar Frete USD', 0.0)
    ...
    if df is not None:
        ...
        if st.session_state.perfil == "dono":
            ...
            with aba_painel:
                ...
                if st.button("🔄 Actualizar Dados do Google Sheets"):
                    st.rerun()
                ...
                with col_tabela2:
                    st.markdown("#### 💵 Posição Financeira Real vs Previsões por Cliente")
                    ...
                    df_resumo_financeiro.columns = ['Cliente', 'Processos', 'Já Recebido', 'Falta Receber (Já Cobrado)', 'Valores Futuros']
                    ...
                    df_resumo_financeiro['Falta Receber (Já Cobrado)'] = df_resumo_financeiro['Falta Receber (Já Cobrado)'].map('$ {:,.2f}'.format)
                    df_resumo_financeiro['Valores Futuros'] = df_resumo_financeiro['Valores Futuros'].map('$ {:,.2f}'.format)
                    ...
                    <div style="display: flex; gap: 15px; justify-content: space-between; flex-wrap: wrap; background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 5px solid #0066cc;">
                        <div style="flex: 1; min-width: 150px; display: flex; flex-direction: column; justify-content: center;">
                            <span style="font-size: 14px; font-weight: 600; color: #555; margin-bottom: 4px;">Total Já Recebido</span>
                            <span style="font-size: 20px; font-weight: bold; color: #2e7d32;">$ {val_recebido:,.2f}</span>
                        </div>
                        <div style="flex: 1; min-width: 150px; display: flex; flex-direction: column; justify-content: center;">
                            <span style="font-size: 14px; font-weight: 600; color: #555; margin-bottom: 4px;">Contas a Receber Real</span>
                            <span style="font-size: 20px; font-weight: bold; color: #c62828;">$ {val_a_receber:,.2f}</span>
                        </div>
                        <div style="flex: 1; min-width: 170px; display: flex; flex-direction: column; justify-content: center;">
                            <span style="font-size: 14px; font-weight: 600; color: #555; margin-bottom: 4px;">Previsão Futura</span>
                            <span style="font-size: 20px; font-weight: bold; color: #1565c0;">$ {val_futuro:,.2f}</span>
                        </div>
                    </div>
                    """
                ...
                st.dataframe(df_subtotal_excel, use_container_width=True, hide_index=
